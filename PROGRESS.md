...
> 最后更新：2026-07-23（WiFi 多账号+离线模式+省电优化+双击唤醒，v0.3.0）
> 状态：**功能全链路可用并已发布 v0.1.0**（GitHub: nasonliu/papers3-weread）。v0.3.0 完成 WiFi 多账号保存（扫到哪个连哪个）、离线模式（WiFi 连不上看已下载的书，手动重连不自动反复重连）、浅睡断 WiFi 省电、双击屏幕唤醒（防误触）。**ESP32 访问 weread 接口间歇性 TCP 连接失败的底层根因仍未闭环**（多为 8s 超时/HTTP 0，同一网络下 Mac curl 全通，换手机热点/时间段有时会自愈）；现有会话保活已消除 UI 假死。下一阶段按 [连接成功率影响因素](#连接成功率影响因素) 继续定位，再进入 [规划中的改进](#规划中的改进-未做)。
...
| WiFi 多账号 | ✅ | v0.3.0：config.json 存 wifi_list 数组，扫描可见 AP 按列表顺序连扫到的（家里/办公室自动切换）；配网门户新账号自动追加 |
| 离线模式 | ✅ | v0.3.0：WiFi 连不上进书架看已下载的书（书架缓存优先），左上角"离线"标记；手动重连（不自动反复重连省电） |
| 省电优化 | ✅ | v0.3.0：浅睡前断 WiFi（wifioff），醒后后台静默重连（不弹 UI）；EPD 浅睡断电 |
| 双击唤醒 | ✅ | v0.3.0：浅睡后双击屏幕唤醒（700ms 窗口+50ms 时间戳去重防误触）；单击继续睡（防口袋误触） |
